<script>
  ...
  $: ariaDuration = Number.isFinite(duration) && duration > 0 ? duration : 0;
  $: ariaCurrentTime = Math.min(Math.max(currentTime, 0), ariaDuration);

  function updateTime() {
    _currentTimePercentage = duration > 0 ? currentTime / duration : 0;
  }

  const updateTimeThrottled = throttle(
    (time) => {
      _currentTimePercentage = duration > 0 ? time / duration : 0;
    },
    250,
    true
  );
...
<div
  ...
  aria-valuetext={`${Math.round(ariaCurrentTime)} seconds`}
  tabindex="0"
